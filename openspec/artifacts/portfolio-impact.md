# Portfolio Impact: mlops-end2end

## Program

- Program id: `mlops-data-platform`
- Program name: MLOps and Data Platform
- Component pack: `mlops-data-platform`

## System Story

Establishes the reusable model lifecycle used by later data quality, feature serving, drift detection, and applied-vision projects.

This repository is not a standalone demo. It is one part of the MLOps and Data Platform system and should produce reusable fixtures, benchmark patterns, and decisions for later repositories.

## Proficiency Signal

- Primary profile: `python`
- Stack profile: `python-ml`
- Stack:
- python-3.12
- apache-airflow-3.3.0
- mlflow-3.14.0
- fastapi-0.136.3
- scikit-learn-1.9.0
- pandera-0.32.1
- prometheus-client-0.25.0
- docker

## Post Angle

Open with time_to_production_seconds_median = pending seconds, then explain why the architecture and local-first path make the result reproducible.
