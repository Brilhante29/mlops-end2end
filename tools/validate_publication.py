...
import jsonschema
from publish_benchmark import ROOT, combined_digest, validate_run
...
def main() -> None:
    ...
    assert provenance["artifact_digest"] == combined_digest(raw_paths)
    assert provenance["dependency_lock_digest"] == combined_digest(
        ["pyproject.toml", "requirements.txt"]
    )
    assert v2["workload"]["fixture_digest"] == combined_digest(
        [
    ...
    assert v2["workload"]["config_digest"] == combined_digest(
        [
            "Dockerfile",
